# Una empresa necesita registrar a sus trabajadores, sus clientes y por ultimo a los administradores del sistema. 
# Los trabajadores tendrán en sus datos: nombre, apellido, profesión, puesto, dni, nombre de usuario y contraseña. 
# Los clientes tendrán solo nombre de usuario y contraseña. El administrador tendrá nombre de usuario, contraseña y rango. 



# 1. Se usa "import json" para poder adquirir los métodos ".dumps" 
# que permitirá converir un diccionario de Python a JSON 
#y el método ".loads" para covertir una cadena JSON a un diccionario de Python:
import json
...
def cargar_datos(): # Función para cargar datos desde el archivo JSON
    ...
    try: # se maneja una excepción
        with open("data.json", "r") as archivo: # se abre el archivo JSON y se indica que está solo en lectura.
            datos = json.load(archivo)
            
    except FileNotFoundError: # La excepción es si no se encuentra el archivo JSON
...
# ______________________________________________________________
# 7. Por ultimo hacemos el menú principal:    
